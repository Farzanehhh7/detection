...
#
# evaluate_binary("SAML-D GraphSAGE، نمونه‌برداری دستی، تک seed", y_test, preds_test, probs_test)
...
class_weight = torch.tensor([1.0, min(raw_weight, 30.0)])  # سقف ۳۰، طبق نتیجه مقایسه step20
print(f"وزن خام: {raw_weight:.2f}   وزن اعمال‌شده با سقف: {class_weight[1].item():.2f}")
print(f"Class weight (illicit): {n_neg / max(n_pos, 1):.2f}")
