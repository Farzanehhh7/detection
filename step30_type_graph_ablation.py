"""
step30_type_graph_ablation.py

Direct test of: "does the system actually use graph RELATIONSHIPS to
determine laundering type, or could the same accuracy be reached with
just the account's own raw numbers, no graph at all?"

Builds a NO-GRAPH type classifier: LogisticRegression directly on the
8 raw account features (no GNN embedding, no structural features, no
message passing of any kind), same merge rule, same illicit-only
train/test split. Then lines it up against the three graph-informed
versions already built:

  no_graph              -- this script:            raw 8 features only
  embedding_only        -- step25:                 GNN embedding (message passing)
  embedding + structural-- step26:                 GNN embedding + explicit degree/cycle features
  multi_task            -- step28:                 GNN trained jointly with the type loss

If embedding_only/enhanced/multi_task clearly beat no_graph, that is
direct evidence the graph relationships are contributing real signal
to the type decision -- not just an assumption because a GNN was used.
"""
